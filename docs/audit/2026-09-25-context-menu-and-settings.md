...
## Vervolgcorrectie: groepen met één meting

De eerste lijstweergave toonde ook voor een groep met precies één ontdekte meting een groepsschakelaar én een identieke ingesprongen metingsschakelaar. Dat maakte CPU-gebruik, CPU-frequentie, CPU-temperatuur en vergelijkbare enkelvoudige sensoren onnodig dubbel. De UI toont nu in dit geval één regel met de eigen verversingsbadge van de meting. De ene schakelaar schrijft zowel de bestaande groepsvoorkeur als de individuele voorkeur, zodat oude configuraties, het opnieuw inschakelen en bestaande HA-entity-ID's blijven werken. Alleen groepen met meerdere metingen krijgen een aparte groepsregel met onderliggende metingen. Frontendtests controleren beide situaties en het opnieuw inschakelen van een voorheen uitgeschakelde groep.

De gebruiker merkte daarnaast terecht op dat de lokale Settings-webview nog het Edge-contextmenu toonde. Ook die webview voorkomt nu het standaardrechtsklikmenu. Een klik op de donkere achtergrond sluit de Settings-dialoog niet meer; X, Annuleren en Escape blijven sluitmogelijkheden. Een UI-test controleert de lokale rechtsklik en de niet-sluitende achtergrond.

De bijgewerkte lokale proefinstaller op phill-pc heeft SHA-256 `17F8F2C7F975815FE456E6B6BA56C3CF0691570871B1108C0C4782036C730785`. Na een bytegelijk geverifieerde back-up bleef `settings.json` door de verhoogde installatie ongewijzigd, de sensordienst draaide en de GUI startte uit Program Files. De app-log toont opnieuw HA-ping `200 OK` en een aangemaakte dashboardwebview. In de echte Settings-weergave zijn CPU-gebruik, CPU-frequentie, CPU-temperatuur en CPU-model enkelvoudige regels zonder dubbele groepsregel. Een rechtsklik in Settings liet geen Edge-menu zien; een daaropvolgende klik op de achtergrond liet de dialoog open. De bestaande sensorwaarden in HA zijn na deze installatie zichtbaar, maar het wijzigen van een individuele sensorvoorkeur is nog niet live tegen HA beproefd. De volledige frontendtests tellen nu 19 geslaagde tests.
