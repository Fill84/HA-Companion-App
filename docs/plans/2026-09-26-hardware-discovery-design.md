...
- Startup discovery caches the Settings reading list, including disabled groups. A later observed change in dynamic reading IDs invalidates the cache. A full snapshot every ten update intervals reconciles disk/network/GPU topology; this is a bounded fallback, not OS event-driven detection.
