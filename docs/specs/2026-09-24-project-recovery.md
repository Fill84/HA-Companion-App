...
De desktopapp moet ook starten wanneer een hardwaredriver niet beschikbaar is. Niet-beschikbare hardwaremetingen worden `unknown` of ontbreken met een expliciete reden; er worden geen schattingen als fysieke sensormetingen gepresenteerd. De Windows-installer levert de gecontroleerde CPU-temperatuurdriver en de eigen sensorservice standaard mee; daarvoor is geen aparte sensorapp of extra keuze in Settings nodig. Op macOS en Linux blijven de eigen, platformspecifieke providers onderdeel van de implementatie. Het HA-protocol moet onderscheid maken tussen HTTP-transport, een herkende opdracht en duurzaam verwerkte data. Het dashboard mag een HA-token uitsluitend aan de exacte, geconfigureerde HA-origin aanbieden. De app belooft geen universele sensordekking; dekking wordt per OS, vendor en provider aangetoond.
