...
!macro NSIS_HOOK_POSTINSTALL
  MessageBox MB_YESNO|MB_DEFBUTTON2|MB_ICONQUESTION "Install the optional CPU temperature provider? This adds the included Microsoft-signed PawnIO driver and the Home Assistant Companion sensor service. Then enable CPU temperature in the app settings. Windows administrator approval is required." IDYES sensor_install_driver IDNO sensor_install_refresh
...
sensor_install_driver:
  ...
  MessageBox MB_ICONSTOP "Could not install the optional sensor driver or service (exit code $0). CPU temperature will remain unknown."
...
sensor_install_refresh:
  ...
  Pop $0
  StrCmp $0 "0" sensor_install_done
  MessageBox MB_ICONSTOP "Could not restart the existing sensor service (exit code $0). CPU temperature will remain unknown."
